...
class delayed_complex_hashset: public complex_hashset_base<T, S, W, H>
{
	...
	struct merge_task
	{
		merge_task(delayed_complex_hashset *executor, chain_info_t & info, size_t first_block_index, typename request_container_t::iterator begin, typename request_container_t::iterator end, std::atomic<int> & new_block_counter, VECTOR_T<std::pair<size_t, chain_info_t>> & new_indices)
	...
	private:
		...
		VECTOR_T<std::pair<size_t, chain_info_t>> & m_newIndices;
...
/*		vector<storage_t::read_request> first_read_req;
		...
		*/
		...
		VECTOR_T<std::pair<size_t, chain_info_t>> new_indices;
	...
	template<typename It>
	void merge_into_chain(chain_info_t & info, size_t first_block_index, It begin, It end, std::atomic<int> & new_block_counter, VECTOR_T<std::pair<size_t, chain_info_t>> & new_indices)
	{
...
private:
	...
	QUEUE_T<insertion_request_t> m_successQueue;
