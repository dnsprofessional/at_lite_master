//tests imported from old project at github.com/ultrablox/ultra_planner
...
void test_is_solved()
{
    ...
    transition_system<edfd_cover> ts(source_graph, vector<edfd_graph>{source_graph}, vector<size_t>{}, false);

	edfd_cover_state state;

	edfd_cover_state final_state = state;
    final_state.fragment_instances.push_back(0);
...
void test_apply_transition()
{
    ...
    transition_system<edfd_cover> ts(source_graph, sdps, vector<size_t>{}, false);
	...
	final_state.cover[2] = 0;
	final_state.cover[3] = 0;
    final_state.main_instances_count = 1;
    ...
    tr.min_distance_from_main_part = std::numeric_limits<size_t>::max();
...
void test_planning_helper(const edfd_graph & source_graph, const vector<edfd_graph>& sdps, const vector<size_t>& main_parts_for_fragments, bool precise, const string & test_name)
{
    using edfd_t = transition_system<edfd_cover>;
    edfd_t ts(source_graph, sdps, main_parts_for_fragments, precise);
...
void test_planning_simple()
{
	...
	vector<edfd_graph> sdps{ source_graph };
    test_planning_helper(source_graph, sdps, vector<size_t>{}, false, "test_planning");
}
...
void test_planning_with_two_subgraphs()
{
    ...
    test_planning_helper(source_graph, sdps, vector<size_t>{}, false, "test_planning_with_two_subgraphs");
}
...
void test_planning_with_included_subgraphs()
{
    ...
    test_planning_helper(source_graph, sdps, vector<size_t>{}, false, "test_planning_with_included_subgraphs");
}
...
void test_planning_real_sdps()
{
    ...
    test_planning_helper(source_graph, sdps, vector<size_t>{}, false, "test_planning_real_sdps");
}
...
//
//	transition_system<edfd_cover> rough_cover(source_graph, vector<edfd_graph>{ main_part_sdp }, vector<size_t>{}, false);
