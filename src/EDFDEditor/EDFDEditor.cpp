...
//	ui.graphicsView->setScene(scene);
   ...
   /* connect(ui.ANewFile, SIGNAL(triggered()), this, SLOT(NewFile()));
	...
	gvDocument = new AWorkspaceWidget(this);
	setCentralWidget(gvDocument);

	auto listWdg = new ADrawWidget(this);
	listWdg->setObjectName("listWidget");
	auto tools_dock = new QDockWidget(tr("Tools"), this);

	tools_dock->setWidget(listWdg);
	tools_dock->setAllowedAreas(Qt::RightDockWidgetArea);
...
void EDFDEditor::showDocument()
{
	...
	gvDocument->setDocument(static_pointer_cast<EDFDDocument>(document()));
	updateScene();
}

void EDFDEditor::updateScene()
{
	gvDocument->scene()->clear();
	...
	for (auto e : static_pointer_cast<EDFDDocument>(document())->getElements())
	{
		...
		case DFDElement::Type::Entity:
			{
				el = new DFDGraphicsEntity(static_pointer_cast<DFDEntity>(e), static_cast<AWorkspaceScene*>(gvDocument->scene()));
				break;
			}
		case DFDElement::Type::Function:
			{
				el = new DFDGraphicsFuntion(static_pointer_cast<DFDFunction>(e), static_cast<AWorkspaceScene*>(gvDocument->scene()));
				break;
			}
		case DFDElement::Type::Storage:
			{
				el = new DFDGraphicsStorage(static_pointer_cast<DFDStorage>(e), static_cast<AWorkspaceScene*>(gvDocument->scene()));
				break;
			}
		case DFDElement::Type::NFFunction:
			{
				el = new DFDGraphicsNFFuntion(static_pointer_cast<DFDNFFunction>(e), static_cast<AWorkspaceScene*>(gvDocument->scene()));
				break;
			}
		default:
			break;
		...
		if(el)
		{
			...
			gvDocument->scene()->addItem(el);
			elem_graph_dictionary[e] = el;

			connect(el, &DFDGraphicsElement::newConnectionRequested, gvDocument, &AWorkspaceWidget::createConnection);
		}
	}
	...
	for (auto c : static_pointer_cast<EDFDDocument>(document())->connections())
	{
		...
		static_cast<AWorkspaceScene*>(gvDocument->scene())->AddConnection(con);
