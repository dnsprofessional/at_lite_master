...
void DFDGraphicsConnection::deleteConnection() {
	
	this->m_pConnection->getDocument().deleteConnection(this->m_pConnection);
	delete this;
}

void DFDGraphicsConnection::changeConnection() {
...
void DFDGraphicsConnection::contextMenuEvent(QGraphicsSceneContextMenuEvent *event)
{
	auto menu = new QMenu;
	QString str = "Delete";
	auto del = new QAction(str, this);
	menu->addAction(del);
	str = "Change";
	auto change = new QAction(str, this);
	menu->addAction(change);
	menu->popup(event->screenPos());
	connect(del, SIGNAL(triggered()), this, SLOT(deleteConnection()));
	connect(change, SIGNAL(triggered()), this, SLOT(changeConnection()));
	/*
	ADatastreamDialog dial(m_pConnection);
	dial.exec();
	*/
