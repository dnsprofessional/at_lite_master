...
class AT_CORE_API EDFDDocument : public AXMLDocument
{
public:
	EDFDDocument();
	...
	bool isDetalizedWith(EDFDDocument * detail_doc) const;
	AError buildDetalizationLinks(std::vector<EDFDDocument*> & edfd_docs);

	void mergeWith(EDFDDocument * detail_doc);
