...
public:
    ...
    friend void calc_ruc_weights(Ontology& onto);
    ...
    std::vector<std::pair<std::string, std::size_t>>& getRucWeights(){
        return m_ruc_weights;
    }
private:
    ...
    std::vector<std::pair<std::string, std::size_t>> m_ruc_weights;
};
