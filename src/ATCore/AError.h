...
{
public:
    //using std::ignore to prevent -Wunused-parameter warning
    virtual void criticalErrorOccured(const AError & error){std::ignore = error;}
    virtual void printMessage(const std::string & msg){std::ignore = msg;}
	AT_CORE_API friend AErrorMessenger & operator<<(AErrorMessenger & os, const std::string & msg);
};
